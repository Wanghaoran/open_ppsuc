...
class CourseAction extends CommonAction {
  ...
  public function course(){
    ...
    $pageNum = !empty($_REQUEST['pageNum']) ? $_REQUEST['pageNum'] : 1;
    $page -> firstRow = ($pageNum - 1) * $listRows;
    $result = $Course -> alias('c') -> field('c.id,c.name,cc.name as ccname,c.addtime,c.content') -> join('open_course_category as cc ON c.cid = cc.id') -> where($where) -> limit($page -> firstRow . ',' . $page -> listRows) -> order('addtime DESC') -> select();
    $this -> assign('result', $result);
    //每页条数
    $this -> assign('listRows', $listRows);
    //当前页数
