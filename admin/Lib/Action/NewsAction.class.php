...
class NewsAction extends CommonAction {
  ...
  public function news(){
    ...
    $pageNum = !empty($_REQUEST['pageNum']) ? $_REQUEST['pageNum'] : 1;
    $page -> firstRow = ($pageNum - 1) * $listRows;
    $result = $News -> alias('n') -> field('n.id,n.title,nc.name as ncname,n.addtime') -> join('open_news_category as nc ON n.cid = nc.id') -> where($where) -> limit($page -> firstRow . ',' . $page -> listRows) -> order('n.addtime DESC') -> select();
    $this -> assign('result', $result);
    //每页条数
    $this -> assign('listRows', $listRows);
    //当前页数
